...
const MakeQuizDesign_D: React.FC = () => {
  ...
  /** 스텝 완료 상태 판단 */
  const steps: StepDef[] = [
    { number: 1, label: t('퀴즈 타입'), isCompleted: true },
    { number: 2, label: t('문제 개수'), isCompleted: true },
    { number: 3, label: t('페이지 선택'), isCompleted: pages.selectedPages.length > 0 },
    { number: 4, label: t('문제 생성'), isCompleted: !!generation.problemSetId },
  ];

  /** 현재 활성 스텝 (첫 번째 미완료 스텝) */
  const activeStep: number = steps.find((s) => !s.isCompleted)?.number ?? 4;

  /** 스텝 인디케이터 컴포넌트 */
  const StepIndicator = ({ step, isLast }: { step: StepDef; isLast: boolean }) => {
        ...
        <div className="flex items-center gap-2 md:flex-col md:gap-1">
          ...
          <div
            className={cn(
              ...
              !step.isCompleted && !isCurrent && 'bg-muted text-muted-foreground',
            )}
          >
            {step.isCompleted ? <CheckCircle className="size-4" /> : step.number}
          </div>
          {/* 텍스트 */}
          <span
            className={cn(
              ...
              isCurrent && !step.isCompleted && 'font-semibold text-primary',
              !step.isCompleted && !isCurrent && 'text-muted-foreground',
            )}
          >
            {step.label}
          </span>
        ...
        {upload.uploadedUrl && !generation.problemSetId ? (
          <div className="lg:grid lg:grid-cols-2 lg:gap-8">
            ...
            <div className="mt-6 space-y-6 lg:mt-0">
              {/* 수평 스텝 인디케이터 */}
              <div className="flex items-center justify-center gap-2 rounded-2xl border border-border bg-background p-4 md:gap-1 md:p-3">
                {steps.map((step, idx) => (
                  <StepIndicator key={step.number} step={step} isLast={idx === steps.length - 1} />
                ))}
              </div>
              ...
              <Card className="rounded-2xl border border-border">
                ...
                <CardContent>
                  {/* 페이지 범위 입력 */}
                  <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-border bg-muted p-5 md:flex-col md:items-stretch md:p-4">
          ...
          <Card className="rounded-2xl border border-border">
            ...
            <CardContent>
              ...
              <div
                className={cn(
                  'flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-muted px-8 py-16 text-center transition-colors duration-200 md:px-5 md:py-10',
                  upload.isDragging && 'border-primary bg-primary/5',
                )}
                onDragOver={uploadActions.handleDragOver}
                onDragEnter={uploadActions.handleDragEnter}
              ...
              >
                ...
                {isWaitingForFirstQuiz && !upload.uploadedUrl ? (
                  ...
                  <>
                    {/* 업로드 아이콘 */}
                    <div className="mb-6 flex size-20 items-center justify-center rounded-2xl border border-dashed border-border bg-background md:size-16">
                      <Cloud className="size-10 text-muted-foreground md:size-8" />
                    </div>
                    <div className="text-lg font-semibold text-foreground md:text-base">
                      {t('파일을 여기에 드래그하세요')}
                    </div>
...
export default MakeQuizDesign_D;
