...
/** V2: Smart State — 상태 반응형 버튼, pill 형태 검토 토글 */
const SolveQuizDesignB_v2: React.FC = () => {
  ...
  // 현재 문제에 답변했는지 여부
  const currentAnswered = !isUnanswered(quiz.currentQuiz.userAnswer, quiz.currentQuiz.selections);
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="mx-auto flex w-[95%] max-w-[1200px] flex-col py-6 lg:grid lg:grid-cols-12 lg:gap-6">
        ...
        <section className="flex flex-col gap-4 lg:col-span-8">
              ...
              <div className="flex w-full items-center rounded-2xl bg-card p-5 shadow-card max-md:flex-col max-md:items-start max-md:gap-3">
                ...
                <div className="border-l border-border pl-4 max-md:self-end max-md:border-l-0 max-md:pl-0">
                  <button
                    ...
                    className={cn(
                      'flex cursor-pointer items-center gap-1.5 rounded-full border-none px-3 py-1.5 text-xs font-semibold transition-all duration-200',
          ...
          {/* V2: Smart State 확인 버튼 — 미선택 시 outline, 선택 시 primary + 체크 아이콘 */}
          <button
            className={cn(
              'mt-auto flex cursor-pointer items-center justify-center gap-2 rounded-2xl border-2 py-3.5 text-base font-medium transition-all duration-200 max-md:mt-4 max-md:w-full',
              currentAnswered
          ...
          {/* V2: 제출하기 버튼 (모바일) */}
          <button
            className="group mt-6 flex cursor-pointer items-center justify-center gap-2 rounded-2xl border-2 border-border bg-transparent py-3 text-sm font-medium text-muted-foreground transition-all duration-200 hover:border-primary hover:bg-primary hover:text-primary-foreground lg:hidden"
        ...
        <aside className="hidden lg:col-span-4 lg:flex lg:flex-col lg:gap-5">
          ...
          <div className="rounded-2xl bg-card p-5 shadow-card">
            ...
            {/* 제출하기 버튼 (데스크톱) */}
            <button
              className="group mt-4 flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl border-2 border-border bg-transparent py-3 text-sm font-medium text-muted-foreground transition-all duration-200 hover:border-primary hover:bg-primary hover:text-primary-foreground"
              onClick={quizActions.handleFinish}
            >
              {t('제출하기')}
              <svg
...
export default SolveQuizDesignB_v2;
