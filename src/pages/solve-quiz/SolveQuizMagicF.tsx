...
/** MagicF: Particle Progress — 진행률에 따라 BlurFade 파티클 시각화, 동적 배경 효과 */
const SolveQuizMagicF: React.FC = () => {
  ...
  /** 진행률 계산 (0~1) */
  const progressRatio = quiz.totalQuestions > 0 ? quiz.answeredCount / quiz.totalQuestions : 0;
  const progressPercent = progressRatio * 100;

  /** 파티클 배열 — 진행률만큼 활성화 */
  const PARTICLE_COUNT = 20;
  const activeParticles = Math.round(progressRatio * PARTICLE_COUNT);

  /** 파티클 위치를 메모이즈 (리렌더마다 랜덤 위치가 바뀌는 것 방지) */
    ...
    return (
      <button
        ...
        className={cn(
          ...
          q.check && 'border-warning bg-warning/15 text-warning',
          q.number === quiz.currentQuestion &&
            'border-primary bg-primary text-primary-foreground shadow-sm hover:scale-100 hover:bg-primary',
        )}
  ...
  const renderPendingButton = (index: number, keyPrefix = ''): React.ReactElement => (
    <button
      ...
      className="flex h-8 w-8 animate-pulse items-center justify-center rounded-full border border-dashed border-border bg-primary/10 text-xs text-primary"
      disabled
    >
      ...
    </button>
  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-background overflow-hidden">
          ...
          >
            <div
              className={cn(
                ...
                p.id < activeParticles ? 'bg-primary/20' : 'bg-muted/30',
              )}
              style={{
                top: `${p.top}%`,
                left: `${p.left}%`,
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          !unanswered && 'bg-primary/20 text-primary',
                          unanswered && 'bg-muted text-muted-foreground',
                          q.check && 'bg-warning/20 text-warning',
                        )}
                      >
                        {q.number}
                      </div>
      ...
      {/* 상단 헤더 */}
      <header className="sticky top-0 z-50 bg-primary/95 text-primary-foreground shadow-sm backdrop-blur-sm">
      ...
      {/* 메인 콘텐츠 */}
      <main className="relative z-10 mx-auto flex w-full max-w-[860px] flex-1 flex-col px-5 py-8 max-md:px-4">
        {quiz.isLoading ? (
          ...
          <div className="flex gap-6 max-md:flex-col">
            {/* 좌측 문제 번호 패널 (데스크톱) */}
            <aside className="flex w-[52px] shrink-0 flex-col gap-2 max-md:hidden">
            ...
            <div className="flex flex-1 flex-col gap-5">
              ...
              {/* 현재 문제 카드 — BlurFade 전환 */}
              <BlurFade key={quiz.currentQuestion} delay={0.05} inView>
                <div className="rounded-2xl bg-card shadow-sm">
                  {/* 문제 헤더 */}
                  <div className="flex items-start justify-between rounded-t-2xl bg-muted px-6 py-5 max-md:flex-col max-md:gap-3">
                    <div className="flex-1 break-words text-base leading-relaxed text-foreground">
                      <MarkdownText>{quiz.currentQuiz.title}</MarkdownText>
                    </div>
                    <label className="ml-4 flex shrink-0 cursor-pointer items-center gap-2 max-md:ml-0">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-sm font-medium text-muted-foreground">{t('검토')}</span>
                    </label>
                  </div>
                  ...
                  <div className="flex flex-col gap-2 p-5">
                    {quiz.currentQuiz.selections.map((opt, idx) => (
                      <BlurFade key={opt.id} delay={0.05 + idx * 0.06} inView>
                        <div
                          className={cn(
      ...
      {/* 하단 문제 번호 패널 (모바일/태블릿) */}
      <aside className="relative z-10 mt-auto hidden grid-cols-[repeat(auto-fill,2rem)] justify-center gap-2 border-t border-border bg-card/80 px-5 py-4 backdrop-blur-sm max-[900px]:grid">
...
export default SolveQuizMagicF;
