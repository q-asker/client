...
const HeaderMagicA_PolishMax = ({
  ...
  // 사이드바 메뉴 항목 정의 (BlurFade stagger용)
  const sidebarMenuItems = [{ type: 'language' as const }, { type: 'help' as const }];
  ...
  return (
    <div className="relative border-b border-border/50 bg-background/80 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-5xl items-center justify-between px-4 py-3 md:px-6">
        ...
        {/* inner glass panel 래퍼 네비게이션 */}
        <div className="flex items-center gap-2">
          <nav className="flex items-center rounded-full bg-muted/20 px-2 py-1 backdrop-blur-sm">
            <div className="flex items-center gap-2">
              <BlurFade delay={0.05}>
                <Link
                  to="/boards"
                  className={cn(
                    'inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm font-medium no-underline transition-all duration-200',
                    isActive('/boards')
                      ? 'border-transparent bg-primary text-primary-foreground shadow-md'
                      : 'border-border/50 bg-background/60 text-muted-foreground backdrop-blur-sm hover:bg-muted hover:text-foreground',
                  )}
                >
          ...
          {isAuthenticated && (
            <div className="relative">
              <button
                id="profileButton"
                className="inline-flex size-8 cursor-pointer items-center justify-center rounded-full border border-border/50 bg-primary/10 p-0 text-sm font-bold text-primary ring-2 ring-primary/20 backdrop-blur-sm transition-colors duration-200 hover:bg-primary/20 md:size-9 md:text-base"
      ...
      {/* 사이드바 + 오버레이 — 그래디언트 배경 + BlurFade stagger */}
      <AnimatePresence>
        {isSidebarOpen && (
          <>
            <motion.div
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 z-[1000] flex h-full w-[280px] flex-col border-r border-border/50 bg-gradient-to-b from-background/95 to-background/85 backdrop-blur-xl"
            >
              ...
              <nav className="flex flex-col gap-0 p-2">
                {sidebarMenuItems.map((item, index) =>
                  item.type === 'language' ? (
                    <BlurFade key="language" delay={index * 0.05}>
                      <motion.div
                        className="flex cursor-pointer items-center justify-between px-4 py-3.5 text-sm text-muted-foreground transition-colors duration-200 hover:text-foreground"
                        custom={index}
                    ...
                    <React.Fragment key="help">
                      ...
                      <BlurFade delay={index * 0.05}>
                        <motion.button
                          className="flex w-full cursor-pointer items-center gap-3 border-none bg-transparent px-4 py-3.5 text-left text-xs font-bold uppercase tracking-[0.15em] text-muted-foreground transition-colors duration-200 hover:text-foreground"
                          type="button"
                          custom={index}
                          variants={sidebarItemVariants}
                          initial="hidden"
...
export default HeaderMagicA_PolishMax;
