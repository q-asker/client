...
const HeaderDesignB_PolishMax = ({
  ...
  return (
    <>
      {/* 헤더 배경에 그라데이션 효과 */}
      <div className="relative bg-gradient-to-r from-background via-muted/10 to-background shadow-sm">
        <div className="mx-auto grid w-full max-w-5xl grid-cols-3 items-center px-4 py-4 md:px-6">
          {/* 좌측: 메뉴 버튼 + 문의하기 — 글래스 효과 pill */}
          <div className="flex items-center gap-1">
            ...
            <BlurFade delay={0.05}>
              <Link
                to="/boards"
                className={cn(
                  'inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium no-underline backdrop-blur-sm transition-all duration-200',
                  isActive('/boards')
                    ? 'bg-background/80 font-semibold text-primary'
          ...
          <div className="flex items-center justify-end gap-1">
            ...
            {/* 프로필 버튼 (인증 시) — 닉네임 표시 추가 */}
            {isAuthenticated && (
              <div className="relative ml-1 flex items-center gap-2">
                <span className="text-sm font-medium text-muted-foreground max-sm:hidden">
                  {displayName}
                </span>
                <button
                  id="profileButton"
...
export default HeaderDesignB_PolishMax;
