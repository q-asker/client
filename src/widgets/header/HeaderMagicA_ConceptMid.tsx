...
const HeaderMagicA_ConceptMid = ({
  ...
  return (
    <div className="relative border-b border-border/50 bg-background/80 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-2">
          <nav className="flex items-center gap-2">
            <BlurFade delay={0.05}>
              <Link
                to="/boards"
                className={cn(
                  'inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm font-medium no-underline ring-1 ring-border/50 transition-all duration-200',
                  isActive('/boards')
                    ? 'border-transparent bg-primary text-primary-foreground shadow-md ring-primary/30'
                    : 'border-border/50 bg-background/60 text-muted-foreground backdrop-blur-sm hover:bg-primary/5 hover:text-foreground hover:backdrop-blur-md',
                )}
              >
                <MessageSquare className="size-4 drop-shadow-sm" />
            ...
            <BlurFade delay={0.1}>
              <div className="relative inline-flex items-center">
                ...
                >
                  <ClipboardList className="size-4 drop-shadow-sm" />
            ...
            {!isAuthenticated && (
              <BlurFade delay={0.15}>
                <Link
                  ...
                  to="/login"
                >
                  <LogIn className="size-4 drop-shadow-sm" />
          ...
          {isAuthenticated && (
            <div className="relative">
              <button
                id="profileButton"
                className="inline-flex size-8 cursor-pointer items-center justify-center rounded-full border border-border/50 bg-primary/10 p-0 text-sm font-bold text-primary backdrop-blur-sm transition-colors duration-200 hover:bg-primary/20 md:size-9 md:text-base"
              ...
              <AnimatePresence>
                {isProfileOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-[calc(100%+8px)] z-[1001] min-w-[180px] rounded-xl border border-border/50 bg-background/90 p-3 shadow-xl backdrop-blur-xl"
                  >
                    <motion.span
        ...
        {isSidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 z-[1000] flex h-full w-[280px] flex-col border-r border-border/50 bg-background/95 backdrop-blur-xl"
            >
              ...
              <nav className="flex flex-col gap-2 p-3">
                <motion.div
                  className="flex cursor-pointer items-center justify-between rounded-lg border border-border/50 bg-background/60 p-3 text-sm text-muted-foreground backdrop-blur-md transition-colors duration-200 hover:text-foreground"
                  custom={0}
                ...
                <motion.button
                  className="flex w-full cursor-pointer items-center gap-3 rounded-lg border border-border/50 bg-background/60 p-3 text-left text-xs font-bold uppercase tracking-[0.15em] text-muted-foreground backdrop-blur-md transition-colors duration-200 hover:text-foreground"
                  type="button"
                  custom={1}
                  variants={sidebarItemVariants}
                  initial="hidden"
...
export default HeaderMagicA_ConceptMid;
