...
/** ConceptMax — 리본을 플로팅 도구모음(toolbar) 형태로 변형 */
const Header = ({ isSidebarOpen, toggleSidebar, setIsSidebarOpen, setShowHelp }: HeaderProps) => {
  ...
  return (
    <div className="relative bg-background">
      {/* 플로팅 툴바 — 상단 중앙에 떠 있는 형태 */}
      <div className="flex items-center justify-center px-3 py-2 md:px-5">
        <div className="flex h-10 items-center gap-1 rounded-full border border-border bg-muted/30 px-2 shadow-sm">
          {/* 메뉴 버튼 */}
          <button
            id="menuButton"
            className="group relative flex size-7 cursor-pointer items-center justify-center rounded-full border-none bg-transparent p-0 text-muted-foreground transition-colors duration-150 hover:bg-background hover:text-foreground"
          ...
          {/* 로고 */}
          <Link to="/" className="flex items-center px-2 text-inherit no-underline">
            <Logo />
          </Link>
          ...
          <Link
            to="/boards"
            className={cn(
              'group relative flex size-7 items-center justify-center rounded-full no-underline transition-colors duration-150',
              isActive('/boards')
          ...
          {!isAuthenticated ? (
            ...
            <div className="relative">
              <button
                id="profileButton"
                className="flex size-7 cursor-pointer items-center justify-center rounded-full border border-border bg-background p-0 text-[10px] font-bold text-foreground transition-all duration-150 hover:border-primary/50 hover:bg-primary/10"
              ...
              <AnimatePresence>
                {isProfileOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-[calc(100%+6px)] z-[1001] min-w-[150px] rounded-lg border border-border bg-background p-1.5 shadow-md"
                  >
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div
            ...
            className="absolute left-3 top-full z-[1000] mt-1 min-w-[200px] rounded-lg border border-border bg-background p-2 shadow-lg md:left-5"
          >
            <div className="flex items-center justify-between px-2 pb-1.5">
              ...
              <button
                className="flex size-5 cursor-pointer items-center justify-center rounded-full border-none bg-transparent p-0 text-muted-foreground transition-colors duration-150 hover:bg-muted hover:text-foreground"
            ...
            <nav className="flex flex-col gap-0.5 py-1.5">
              <div className="flex items-center justify-between rounded-md px-2 py-1.5 text-muted-foreground">
              ...
              <button
                className="flex w-full cursor-pointer items-center gap-2 rounded-md border-none bg-transparent px-2 py-1.5 text-left text-[11px] font-semibold uppercase tracking-wide text-muted-foreground transition-colors duration-150 hover:bg-muted hover:text-foreground"
                type="button"
                onClick={handleHelp}
              >
