...
const HeaderDesignB_PolishMid = ({
  ...
  return (
    <>
      ...
      <div className="relative bg-background shadow-sm">
        <div className="mx-auto grid w-full max-w-5xl grid-cols-3 items-center px-4 py-4 md:px-6">
          ...
          <div className="flex items-center gap-1">
            ...
            <BlurFade delay={0.05}>
              <Link
                to="/boards"
                className={cn(
                  'inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium no-underline transition-all duration-200 hover:shadow-sm hover:scale-[1.02]',
                  isActive('/boards')
          ...
          <div className="flex justify-center">
            <Link
              to="/"
              className="rounded-full bg-muted/30 px-4 py-1 text-inherit no-underline transition-colors duration-200"
            >
              <Logo />
            </Link>
          </div>
          ...
          <div className="flex items-center justify-end gap-1">
            ...
            {/* 로그인 버튼 (미인증 시) — hover 시 shadow + scale */}
            {!isAuthenticated && (
              <BlurFade delay={0.15}>
                <Link
                  className={cn(
      ...
      {/* 사이드바 + 오버레이 — 열림/닫힘 transition duration-300 ease-out */}
      <AnimatePresence>
        {isSidebarOpen && (
          <>
            <motion.div
...
export default HeaderDesignB_PolishMid;
