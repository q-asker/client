...
const HeaderDesignB_ConceptMid = ({
  isSidebarOpen,
  toggleSidebar,
  setIsSidebarOpen,
  setShowHelp,
  ...
  const currentLang = useMemo(() => {
    const lang = (typeof t === 'function' && t('lang_code')) || 'ko';
    return lang === 'lang_code' ? 'ko' : lang;
  }, [t]);
    ...
    <>
      <div className="relative bg-background shadow-sm">
        ...
        <div className="mx-auto grid w-full max-w-5xl grid-cols-3 items-center px-4 py-4 md:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <BlurFade delay={0.05}>
              <Link
                to="/boards"
                className={cn(
                  'inline-flex items-center rounded-lg px-3 py-2 text-sm font-medium no-underline transition-all duration-200',
                  isActive('/boards')
                    ? 'bg-muted font-semibold text-primary ring-2 ring-primary/30'
          ...
          {/* 우측: 퀴즈 기록 + 로그인/프로필 — pill 간격 gap-2 */}
              ...
              <nav className="flex flex-col gap-0 p-2">
                ...
                >
                  ...
                  <div className="flex rounded-lg bg-muted p-1">
                    <button
                      className={cn(
                        'cursor-pointer rounded-md border-none px-2.5 py-1 text-xs font-bold transition-all duration-200',
                        currentLang === 'ko'
...
export default HeaderDesignB_ConceptMid;
