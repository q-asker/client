...
- **`src/app/globals.css`의 `@theme` 블록에 정의된 CSS 변수만 사용** — 임의의 색상값(HEX, RGB, oklch 직접 지정)이나 Tailwind 기본 팔레트(slate, blue 등) 금지. 새 색상이 필요하면 globals.css에 먼저 정의한 후 사용한다
- **tweakcn 제공 색상만 사용** — 임의의 CSS 변수를 새로 만들지 않는다
- tweakcn이 제공하는 시맨틱 변수만 사용: `background`, `foreground`, `primary`, `secondary`, `accent`, `muted`, `destructive`, `card`, `popover`, `border`, `input`, `ring`, `chart-1`~`chart-5`, `sidebar-*` 및 각 `*-foreground` 쌍
- 색상 변형이 필요하면 Tailwind 불투명도 수정자(`/opacity`)로 조합한다 (예: `bg-chart-2/12 text-chart-2`)
...
```tsx
/* ❌ 커스텀 CSS 변수 생성 금지 */
--color-badge-success: oklch(0.52 0.09 170);  /* 임의 색상 */

/* ✅ tweakcn 제공 변수 + 불투명도 수정자 조합 */
<span className="bg-chart-2/12 text-chart-2">완료</span>   /* teal */
<span className="bg-chart-5/12 text-chart-5">미완료</span>  /* rose */
<span className="bg-chart-3/12 text-chart-3">검토</span>    /* gold */
<span className="bg-primary/10 text-primary">강조</span>    /* primary */
<span className="bg-destructive/10 text-destructive">에러</span>
```
