...
- **마크다운**: react-markdown + remark-gfm + mermaid (다이어그램) + rehype-highlight (코드 하이라이팅)
...
npm run i18n:extract   # 번역 키 추출
npm run i18n:type      # 타입 생성
npm run i18n:sync      # wrap + extract + type 일괄 실행
